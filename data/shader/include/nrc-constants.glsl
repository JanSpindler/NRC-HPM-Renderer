...
const uint HIDDEN_WEIGHTS_COUNT = LAYER_WIDTH * LAYER_WIDTH * (LAYER_COUNT - 1);
...
const uint RENDER_OUTPUT_NEURONS_INDEX = RENDER_SAMPLE_COUNT * (INPUT_FEATURE_COUNT + (LAYER_WIDTH * LAYER_COUNT));
const uint TRAIN_OUTPUT_NEURONS_INDEX = TRAIN_SAMPLE_COUNT * (INPUT_FEATURE_COUNT + (LAYER_WIDTH * LAYER_COUNT));

const uint NV_TILE_SIZE = 16;
const uint NV_TILES_PER_BATCH = BATCH_SIZE / NV_TILE_SIZE;

#define mFCoopmatNV fcoopmatNV<16, gl_ScopeSubgroup, NV_TILE_SIZE, NV_TILE_SIZE>
